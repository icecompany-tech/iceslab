 ...
 *
 * `installed` в контракте ещё нет, он придёт у `CoreStatus` вместе с
 * `reservedPorts`. Тесты ниже описывают поведение, которое должно пережить его
 * появление: отсутствие поля это «агент старше поля», а не «не установлено».
 */
...
function core(p: Partial<NodeCore> & { installed?: boolean }): NodeCore {
