...
/**
 * Нога ДО ВЫХОДА, своя у каждого направления.
 *
 * Три значения поля, и первые два разные. `undefined` это «сервер поля не
 * прислал»: строка показывает ячейку входа, селектор заблокирован, и экран
 * молчит про настройку, которой не видел. `null` это «поле есть, ячейка не
 * выбрана», направление идёт ячейкой входа сознательно. Разница между «нечего
 * сказать» и «не выбрано» тут и есть весь смысл строки.
 *
 * Поля приехали с фазой 5 (`c71788d`), но проверка на `undefined` осталась и
 * остаётся насовсем: поле может не прийти и по другим причинам, от панели
 * постарше против этого фронта до узкой выборки в маппере. Пустой селектор
 * вместо молчания читался бы как «оператор не выбрал», то есть врал бы о
 * причине.
 *
...
export function DirectionLegRow({
  ...
  /** Прислал ли сервер поля ноги направления. `false` = не прислал, причина
   *  экрану неизвестна и выдумывать её он не будет. */
  available: boolean;
    ...
    <Stack gap={8} style={{ width: '100%', paddingLeft: 14 }}>
      <Box className="cascade-leg" style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}>
        ...
        <Box style={{ flex: 1, minWidth: 0 }}>
          {!available && (
            <Text style={{ fontFamily: DISPLAY, fontSize: 11, lineHeight: '15px', color: FAINT }}>
              {t('cascadeCreate.legNotReported')}
            </Text>
          )}
        </Box>
        <Box style={{ width: 210, flexShrink: 0 }}>
