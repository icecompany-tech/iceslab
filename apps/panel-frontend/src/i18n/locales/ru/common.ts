export const common = {
  ...
  nodeCore: {
    ...
    gateAwgSubnet:
      'На ноде {{node}} подсеть {{subnet}} пересекается с {{otherSubnet}} профиля {{other}}: у профилей 1.x и 3.1 на одной ноде подсети должны различаться.',
  ...
  cascades: {
    ...
    // E57: направление на именованном выходе.
    directionViaOutbound: 'выход',
    directionOutboundGone: 'выход не найден',
    directionDown: 'вне',
    outShort: 'вне',
    reaches: 'Доступен',
  ...
  chain: {
    ...
    // E57: связи с нодой нет, это последний отчёт, а не текущее знание.
    staleUp: 'По последнему отчёту цепь работала',
    staleUpVersion: 'По последнему отчёту цепь работала · {{version}}',
    staleDown: 'По последнему отчёту цепь не работала',
    staleSince: '(связь с нодой потеряна {{when}})',
