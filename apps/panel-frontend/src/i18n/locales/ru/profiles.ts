export const profiles = {
  ...
  profiles: {
    ...
    telegramPreview: {
      ...
      bannerHint:
        'Поля живые, но значения остаются на этой странице: бэкенд такой вход ещё не принимает, ничего не отправляется и сервером не проверяется. Профиль для Telegram прямо сейчас создаёт MTProto.',
      ...
      socks5: {
        ...
        authNone: 'Без пароля',
        authNote:
          'RFC 1929. Логин и пароль тут не вводятся: у каждого пользователя свои, имя и его UUID.',
        portNote: 'Порт, который нода откроет под SOCKS5. Ссылка tg://socks есть у всех трёх клиентов Telegram.',
        udpLabel: 'UDP associate',
        udpOn: 'Включено',
        udpOff: 'Выключено',
        udpNote: 'Выключено по умолчанию: звонки Telegram через SOCKS5 UDP не ходят, клиент UDP associate не использует.',
        warn: 'Без обфускации: для сетей, где прокси разрешён, не для обхода DPI. SOCKS5 сам ничего не шифрует, а открытый в интернет без пароля становится общим прокси для всех, кто знает адрес.',
      },
      http: {
        title: 'HTTP CONFIG',
        authLabel: 'Способ авторизации',
        ...
        authNone: 'Без пароля',
        authNote:
          'Логин и пароль тут не вводятся: у каждого пользователя свои, имя и его UUID. Digest не поддерживается ни ядром, ни клиентом Telegram.',
        portNote: 'Туннель CONNECT, только TCP: звонки через HTTP-прокси не пойдут.',
        clients: 'Только Telegram Desktop, ссылки для добавления у Telegram нет: адрес и порт человек вводит руками.',
        warn: 'Без обфускации: для сетей, где прокси разрешён, не для обхода DPI. Basic это логин и пароль в base64, а не шифрование, без TLS сверху их читает любой на пути.',
      },
      web: {
