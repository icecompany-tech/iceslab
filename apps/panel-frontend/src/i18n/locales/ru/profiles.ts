export const profiles = {
  ...
  profiles: {
    ...
    form: {
      ...
      cfg: {
        ...
        // Поколение AWG у профиля (227054e): выбор профиля, модуль ноды факт.
        awgGenerationLabel: 'Поколение AmneziaWG',
        awgGenerationHint: '3.1 нужен модуль 3.1 на ноде и клиент AmneziaVPN 5.x.',
        // До t07-6: профиль 3.1 хранится, раздача 3.1 приедет позже.
        awgGenerationPending: 'Поколение 3.1 будет отдаваться после следующих шагов фазы 7.',
  ...
  recipes: {
    ...
    emptyForKind: 'В реестре рецептов для {{kind}} нет. Импортируйте свой или предложите его в реестр.',
    // Строка панели рецептов (владелец, 25.09): рецепты только из реестра.
    fromRegistry: 'Рецепты из реестра',
    proposeOwn: 'предложите свой:',
    // Реестр не ответил, снимка у сервера нет (сервер старше снимка).
    registryUnavailable:
      'Реестр рецептов недоступен, а снимка реестра у этой панели нет: показать нечего. Импорт по ссылке или JSON работает.',
    // Свои и скрытые (контракт 25.09).
    mine: {
      badge: 'мой',
      ...
      alreadyGone: 'Этого рецепта уже нет: список перечитан.',
    },
    hidden: {
      hide: 'Скрыть',
      expand_one: 'скрыт {{count}}, показать',
      expand_few: 'скрыто {{count}}, показать',
      expand_many: 'скрыто {{count}}, показать',
      expand_other: 'скрыто {{count}}, показать',
      collapse_one: 'скрыт {{count}}, свернуть',
      collapse_few: 'скрыто {{count}}, свернуть',
      collapse_many: 'скрыто {{count}}, свернуть',
      collapse_other: 'скрыто {{count}}, свернуть',
      restore: 'Вернуть',
    },
    alsoIn: 'также в: {{list}}',
    // Сервер сливает дубли сам; два рецепта с одним id это его ошибка.
    duplicates: 'Сервер прислал два рецепта с одним id ({{ids}}): это ошибка, сообщите о ней.',
    ...
    import: {
      ...
      hint: 'Вставь ссылку на рецепт (страница файла GitHub, gist или raw) или JSON рецепта. Он валидируется, потом выбираешь какой применить.',
      urlLabel: 'URL рецепта',
      jsonLabel: 'или вставь JSON рецепта',
      load: 'Загрузить',
      pick: 'Выбери, что применить:',
      none: 'Валидных рецептов не найдено.',
      // Страница GitHub или gist переведена в raw (recipeImportUrl).
      rewritten: 'Будет загружено: {{url}}',
      gistFirstFile: 'Из gist берётся первый файл; для другого вставьте ссылку с его кнопки Raw.',
      failed: 'Импорт не удался',
      wrongProtocol: 'Для плитки {{protocol}} там ничего нет (сюда применимы только её рецепты).',
      hidden: 'Скрыто рецептов для других плиток: {{count}}.',
    },
    export: {
      ...
      // Экспорт сохранённого профиля (GET /api/profiles/:id/recipe, 25.09).
      fromSaved:
        'Рецепт собран сервером из сохранённого профиля: несохранённые правки формы в него не входят. Случайные значения (пароли, заголовки AmneziaWG) уходят как «генерировать при применении», а не значениями этого профиля. Ничего не сохраняется.',
      pathHint: 'Сохраните как файл в форке реестра:',
      ratingsHint: 'Оценки DPI и скорости выставлены 3 из 5: поправьте их перед pull request, панель их не измеряет.',
      copy: 'Копировать',
      copied: 'Скопировано',
      failed: 'Собрать рецепт не вышло: {{message}}',
