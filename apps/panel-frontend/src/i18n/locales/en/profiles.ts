export const profiles = {
  ...
  profiles: {
    ...
    form: {
      ...
      cfg: {
        ...
        awgGenerationLabel: 'AmneziaWG generation',
        awgGenerationHint: '3.1 needs the 3.1 module on the node and the AmneziaVPN 5.x client.',
        awgGenerationPending: 'Generation 3.1 will be handed out after the next steps of phase 7.',
  ...
  recipes: {
    ...
    mine: {
      ...
      alreadyGone: 'This recipe is already gone: the list was reloaded.',
    },
    hidden: {
      hide: 'Hide',
      expand_one: '{{count}} hidden, show',
      expand_other: '{{count}} hidden, show',
      collapse_one: '{{count}} hidden, collapse',
      collapse_other: '{{count}} hidden, collapse',
      restore: 'Restore',
    },
    alsoIn: 'also in: {{list}}',
    duplicates: 'The server sent two recipes with one id ({{ids}}): that is a bug, please report it.',
    ...
    import: {
      ...
      hint: 'Paste a link to a recipe (a GitHub file page, a gist or a raw URL) or the recipe JSON. It is validated, then you pick one to apply.',
      urlLabel: 'Recipe URL',
      jsonLabel: 'or paste recipe JSON',
      load: 'Load',
      pick: 'Pick one to apply:',
      none: 'No valid recipes found.',
      rewritten: 'Will load: {{url}}',
      gistFirstFile: 'A gist gives its first file; for another one paste the link from its Raw button.',
      failed: 'Import failed',
      wrongProtocol: 'Nothing in there for the {{protocol}} tile (only its recipes apply here).',
      hidden: '{{count}} recipe(s) for other tiles hidden.',
    },
    export: {
