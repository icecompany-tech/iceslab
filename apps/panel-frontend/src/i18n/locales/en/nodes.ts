export const nodes = {
  nodes: {
    ...
    form: {
      ...
      enginesRefusedToast: 'The server did not accept the node cores ({{code}}, {{field}}): {{message}}',
      enginesHint: 'Every ticked core is installed. Cores can be added or removed later, on the node page.',
      coreVersionsBuilt: 'built on the node from source (the @naive branch), no release to choose',
  ...
  nodeCreate: {
    ...
    canAttachHint: 'These work right away on a {{engines}} node',
  ...
  nodeEdit: {
    ...
    paramsTitle: 'Parameters',
    protocolDesc: 'Main core of the node.',
    intendedEnginesHint: 'The cores the node is set up to carry. What the machine actually has is in the Cores section.',
    addressDesc:
    ...
    coresAwgFact: {
      awg3: 'AWG 3.1: carries 1.x and 3.1 profiles',
    ...
    coreVer: {
      ...
      choice: {
        ...
        pin: 'Pin {{v}}',
        pinCurrent: 'Pin {{v}} (recommended now)',
      },
      choiceNote:
        'The recommended one moves with the panel manifest, a pinned one stays until you decide. Either way the core on the node changes only by the command from «Cores».',
    ...
    hyTls: {
      acme: 'ACME certificate for {{host}}',
      selfSigned: 'self-signed, fingerprint {{fp}}…',
      match: 'matches the panel',
      mismatch: 'does not match: the node is on the old certificate, waiting for a push',
      until: 'until {{date}}',
      unreported: 'the agent did not report its certificate',
      rotate: 'Rotate certificate',
      rotateConfirm: 'Every hy2 link handed out stops connecting until clients refresh their subscription. Rotate?',
      rotated: 'A new certificate was issued and sent to the node',
      rotateFailed: 'Certificate not rotated',
      notSelfSigned: 'The node has a domain name: its certificate is ACME, nothing to rotate',
    },
